...
def main():
    ...
    # Calculate total flu positives (NREVSS approach)
    flu_columns = ['A (2009 H1N1)', 'A (H3)', 'A (Subtyping not Performed)', 'B']
    df_flu['total_flu_positives'] = df_flu[flu_columns].sum(axis=1)
    ...
    flu_data_columns = ['REGION TYPE', 'REGION', 'YEAR', 'WEEK', 'TOTAL SPECIMENS', 
                       ...
                       'B', 'BVic', 'BYam', 'H3N2v', 'A (H5)', 'PERCENT POSITIVE', response_var]
    ...
    plt.tight_layout()
    plt.savefig(f'ShiHaoYang/Results/New/granger_causality_robust_analysis_nrevss_max_lag{max_lag}.png', dpi=300, bbox_inches='tight')
    plt.show()
